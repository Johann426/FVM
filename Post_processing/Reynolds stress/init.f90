	...
	ALLOCATE(EDDY_CONDUC(IS:IE,JS:JE,KS:KE),STAT=ALLOC_ERR)
	...
	IF(POS_LES.EQ.1) THEN
		ALLOCATE(U_PRM(IS:IE,JS:JE,KS:KE),STAT=ALLOC_ERR)
		ALLOCATE(V_PRM(IS:IE,JS:JE,KS:KE),STAT=ALLOC_ERR)
		ALLOCATE(W_PRM(IS:IE,JS:JE,KS:KE),STAT=ALLOC_ERR)
		ALLOCATE(T_PRM(IS:IE,JS:JE,KS:KE),STAT=ALLOC_ERR)
		
		U_PRM(IS:IE,JS:JE,KS:KE) = 0.D0
		V_PRM(IS:IE,JS:JE,KS:KE) = 0.D0
		W_PRM(IS:IE,JS:JE,KS:KE) = 0.D0
		T_PRM(IS:IE,JS:JE,KS:KE) = 0.D0
	ENDIF
	...
	IF(IRESTART.EQ.0.AND.RANK.EQ.0)	THEN
	    ...
	    OPEN(UNIT=12,FILE='CS.PLT',STATUS='UNKNOWN')
	    WRITE(12,*) 'variables=TIME,Cs'
	    CLOSE(12)
	    
	    OPEN(UNIT=13,FILE='CT.PLT',STATUS='UNKNOWN')
	    WRITE(13,*) 'variables=TIME,Ct'
	    CLOSE(13)
	...
	IF(IRESTART.EQ.1) THEN
		...
		READ(16+RANK,122) DUM,DUM,DUM,RTIME,NITER_TIME	!,DTIME,RTIME,NITER_TIME
122		FORMAT(TR1,E24.16,TR1,E24.16,TR1,E24.16,TR1,E24.16,TR1,I16)
		DO K=KS,KE
		DO J=JS,JE
		DO I=IS,IE
			READ(16+RANK,128) NLX(I,J,K),NLY(I,J,K),NLZ(I,J,K),ADV(I,J,K)
			READ(16+RANK,129) U(I,J,K),V(I,J,K),W(I,J,K),P(I,J,K),T(I,J,K)
128		FORMAT(TR1,E24.16,TR1,E24.16,TR1,E24.16,TR1,E24.16)
129		FORMAT(TR1,E24.16,TR1,E24.16,TR1,E24.16,TR1,E24.16,TR1,E24.16)
		ENDDO
		ENDDO
		ENDDO
		CLOSE(16+RANK)
	...
	IF(POS_LES.EQ.1) THEN
		...
		OPEN(UNIT=48+RANK,FILE=FILENAME2,STATUS='OLD')
		READ(48+RANK,224) DUM,DUM,DUM,RTIME,NITER_POS
224		FORMAT(TR1,D16.9,TR1,D16.9,TR1,D16.9,TR1,D16.9,TR1,I16)
		DO K=KS,KE
		DO J=JS,JE
		DO I=IS,IE
			READ(48+RANK,229) U_AVG(I,J,K),V_AVG(I,J,K),W_AVG(I,J,K),T_AVG(I,J,K)
229		FORMAT(TR1,D16.9,TR1,D16.9,TR1,D16.9,TR1,D16.9)
		ENDDO
		ENDDO
		ENDDO
		CLOSE(16+RANK)
