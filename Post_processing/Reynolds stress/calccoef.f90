...
!   SUBROUTINE FOR POST-PROCESSING
    IF(IPOST.EQ.1) THEN
		...
		W_RMS(IS:IE,JS:JE,KS:KE) = W_RMS(IS:IE,JS:JE,KS:KE) + W(IS:IE,JS:JE,KS:KE)**2
	ENDIF
!---------------------------------------------------------------------------------------------

    IF(POS_LES.EQ.1) THEN
		NITER_POS_LES = NITER_POS_LES + 1
		U_PRM(IS:IE,JS:JE,KS:KE) = U_PRM(IS:IE,JS:JE,KS:KE) + U(IS:IE,JS:JE,KS:KE) - U_AVG(IS:IE,JS:JE,KS:KE)
		V_PRM(IS:IE,JS:JE,KS:KE) = V_PRM(IS:IE,JS:JE,KS:KE) + V(IS:IE,JS:JE,KS:KE) - V_AVG(IS:IE,JS:JE,KS:KE)
		W_PRM(IS:IE,JS:JE,KS:KE) = W_PRM(IS:IE,JS:JE,KS:KE) + W(IS:IE,JS:JE,KS:KE) - W_AVG(IS:IE,JS:JE,KS:KE)
		T_PRM(IS:IE,JS:JE,KS:KE) = T_PRM(IS:IE,JS:JE,KS:KE) + T(IS:IE,JS:JE,KS:KE) - T_AVG(IS:IE,JS:JE,KS:KE)
	ENDIF
