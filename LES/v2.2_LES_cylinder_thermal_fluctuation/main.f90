...
!	  Three dimensional viscous flow past a cylinder				!
!											!
!	- FVM : Cell center scheme							!
!	- Fractional 4-Step method							!
!	- Time-Dependent, incompressible, laminar flow				!
!	- Constant materials								!
!	- Based on colocated grid arrangement(non-staggerd grid)			!
!											!
!	  EXTERNAL FLOW with dynamic LES model		v2.1, Jin-Wook Lee	!
!											!
!							Last Modified : Sep.01, 2010	!
!***********************************************************************************!
...
!	ADVECTIVE EXPLICIT TERM : CONSERVATIVE FORM
	...
	CALL NL(W_N,NLZ)
	CALL NL(T_N,ADV)
	IF(USE_LES.EQ.1) THEN !	 EXPLICITLY TREATED SUBGRID-SCALE STRESS
		CALL STRESS_TENSOR_DY
		NLX(IS1:IE1,JS1:JE1,KS1:KE1)= NLX(IS1:IE1,JS1:JE1,KS1:KE1)	&
									+ SGS_X(IS1:IE1,JS1:JE1,KS1:KE1)*DJAC(IS1:IE1,JS1:JE1,KS1:KE1)
		NLY(IS1:IE1,JS1:JE1,KS1:KE1)= NLY(IS1:IE1,JS1:JE1,KS1:KE1)	&
									+ SGS_Y(IS1:IE1,JS1:JE1,KS1:KE1)*DJAC(IS1:IE1,JS1:JE1,KS1:KE1)
		NLZ(IS1:IE1,JS1:JE1,KS1:KE1)= NLZ(IS1:IE1,JS1:JE1,KS1:KE1)	&
									+ SGS_Z(IS1:IE1,JS1:JE1,KS1:KE1)*DJAC(IS1:IE1,JS1:JE1,KS1:KE1)
		ADV(IS1:IE1,JS1:JE1,KS1:KE1)= ADV(IS1:IE1,JS1:JE1,KS1:KE1)	&
									+ SGS_T(IS1:IE1,JS1:JE1,KS1:KE1)*DJAC(IS1:IE1,JS1:JE1,KS1:KE1)
	ENDIF
...
!	ENDIF
!	
!	CALL MPI_ALLREDUCE(DUM_B,DUM_A,1,MPI_DOUBLE_PRECISION,MPI_SUM,NMCW,ERR)	! REF. PRESSURE ON THE INLET BOUNDARY
!	
!	P(IS:IE,JS:JE,KS:KE)=P(IS:IE,JS:JE,KS:KE) - DUM_A / NDIMK
	
	DUM_B = 0.D0
	IF(JS.LT.NJ/4 .AND. JE.GT.NJ/4) THEN
		DO K=KS1,KE1
			DUM_B = DUM_B + P(IE,NJ/4,K)
		ENDDO
			DUM_B = DUM_B/DBLE(KE1-KS1+1)
	ENDIF
	
	CALL MPI_ALLREDUCE(DUM_B,DUM_A,1,MPI_DOUBLE_PRECISION,MPI_SUM,NMCW,ERR)	! REF. PRESSURE ON THE FAR FIELD BOUNDARY
