...
!	CALCULATION OF DUDN
	...
	IF(IPOST.EQ.1) THEN
		SH_S(I,J,K) = SH_S(I,J,K) + DUDN*AREA1(I,J,K)/(AREA1(I,J,K)+AREA2(I,J,K))
...
!		CP_S(I,J,K)	= CP_S(I,J,K)
	...
	OPEN(UNIT=11,FILE='CD_CL.PLT',STATUS='OLD',ACCESS='APPEND')
	WRITE (11,*) RTIME,CD,CL,CPB,CD_P,CD_V,CL_P,CL_V,CFL_MAX ! PLOTTING
	CLOSE(11)
...
!   SUBROUTINE FOR POST-PROCESSING
    IF(IPOST.EQ.1) THEN
		...
		U_AVG(IS:IE,JS:JE,KS/2+KE/2) = U_AVG(IS:IE,JS:JE,KS/2+KE/2) + U(IS:IE,JS:JE,KS/2+KE/2)
		U_RMS(IS:IE,JS:JE,KS/2+KE/2) = U_RMS(IS:IE,JS:JE,KS/2+KE/2) + U(IS:IE,JS:JE,KS/2+KE/2)**2
!		U_TA(:,:,:) = U_TA(:,:,:) + U(:,:,:)
!		V_TA(:,:,:) = V_TA(:,:,:) + V(:,:,:)
!		W_TA(:,:,:) = W_TA(:,:,:) + W(:,:,:)
!		P_TA(:,:,:) = P_TA(:,:,:) + P(:,:,:)
