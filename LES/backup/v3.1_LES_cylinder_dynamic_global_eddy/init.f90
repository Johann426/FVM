...
3   FORMAT(TR1,'Re =',F10.2,TR4,'Pr =',F7.2,TR4,'Pe =',F7.2,TR4,'dtime =',F10.4,TR12)
	...
	LX	= MAX(XG(I,J,K),XG(I+1,J,K),XG(I,J+1,K),XG(I+1,J+1,K),XG(I,J,K+1),XG(I+1,J,K+1),XG(I,J+1,K+1),XG(I+1,J+1,K+1))	&
		- MIN(XG(I,J,K),XG(I+1,J,K),XG(I,J+1,K),XG(I+1,J+1,K),XG(I,J,K+1),XG(I+1,J,K+1),XG(I,J+1,K+1),XG(I+1,J+1,K+1))
	LY	= MAX(YG(I,J,K),YG(I+1,J,K),YG(I,J+1,K),YG(I+1,J+1,K),YG(I,J,K+1),YG(I+1,J,K+1),YG(I,J+1,K+1),YG(I+1,J+1,K+1))	&
		- MIN(YG(I,J,K),YG(I+1,J,K),YG(I,J+1,K),YG(I+1,J+1,K),YG(I,J,K+1),YG(I+1,J,K+1),YG(I,J+1,K+1),YG(I+1,J+1,K+1))
	LZ	= MAX(ZG(I,J,K),ZG(I+1,J,K),ZG(I,J+1,K),ZG(I+1,J+1,K),ZG(I,J,K+1),ZG(I+1,J,K+1),ZG(I,J+1,K+1),ZG(I+1,J+1,K+1))	&
		- MIN(ZG(I,J,K),ZG(I+1,J,K),ZG(I,J+1,K),ZG(I+1,J+1,K),ZG(I,J,K+1),ZG(I+1,J,K+1),ZG(I,J+1,K+1),ZG(I+1,J+1,K+1))
	
	DUM = DJAC(I,J,K)**(1.0D0/3.0D0)
	
	WID1(I,J,K)=DJAC(I,J,K)**(1.0D0/3.0D0)	!DUM*LX/(LX*LY*LZ)**(1.0D0/3.0D0)	! ACTUALLY WID1=DX
	WID2(I,J,K)=DJAC(I,J,K)**(1.0D0/3.0D0)	!DUM*LY/(LX*LY*LZ)**(1.0D0/3.0D0)	! ACTUALLY WID2=DY
	WID3(I,J,K)=DJAC(I,J,K)**(1.0D0/3.0D0)	!DUM*LZ/(LX*LY*LZ)**(1.0D0/3.0D0)	! ACTUALLY WID3=DZ
	
	T_VOLUME = T_VOLUME + DJAC(I,J,K)
	
	ENDDO
	ENDDO
	ENDDO
		CALL MPI_ALLREDUCE(T_VOLUME,DUM,1,MPI_DOUBLE_PRECISION,MPI_SUM,NMCW,ERR)
		T_VOLUME = DUM
		WRITE(*,*) 'TOTAL VOLUME =',T_VOLUME
	ENDIF
!--------------------------------------------------------------------------------------
	END SUBROUTINE INIT