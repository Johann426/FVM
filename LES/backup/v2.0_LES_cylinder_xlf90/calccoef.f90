...
!	CALCULATION OF DUDN
	...
	IF(DSIN(THETA(I,J,K))*DUM_B.LT.0.D0 .OR. DCOS(THETA(I,J,K))*DUM_A.LT.0.D0) THEN
		DUM_A = -1.D0*DUM_A
		DUM_B = -1.D0*DUM_B
		DUM_C = -1.D0*DUM_C
		DUM_D = -1.D0*DUM_D
	...
	DUDN =	+U(I,J,K)*DUM_B/DSQRT(DUM_A**2+DUM_B**2)	&
			-V(I,J,K)*DUM_A/DSQRT(DUM_A**2+DUM_B**2)	&
			-W(I,J,K)*DUM_A/DSQRT(DUM_A**2+DUM_C**2)	&
			-VEL_ROT
	...
	T_DRAG = T_DRAG + (2.0D0/RE)*DUDN*AREA1(I,J,K)*DSIN(THETA(I,J,K))
					
	T_LIFT_Y = T_LIFT_Y - (2.0D0/RE)*DUDN*AREA1(I,J,K)*DCOS(THETA(I,J,K))
	...
	IF(IPOST.EQ.1) THEN
		SH_S(I,J,K) = SH_S(I,J,K) + DABS(DUDN)*AREA1(I,J,K)/(AREA1(I,J,K)+AREA2(I,J,K))
		NU_S(I,J,K)	= NU_S(I,J,K) + DTDN*AREA1(I,J,K)/(AREA1(I,J,K)+AREA2(I,J,K))
		CP_S(I,J,K)	= CP_S(I,J,K) + 0.125D0*(P(I,J,K)+P(I,J-1,K)+P(I-1,J,K)+P(I-1,J-1,K)+P(I,J,K+1)+P(I,J-1,K+1)+P(I-1,J,K+1)+P(I-1,J-1,K+1))
	ENDIF
	...
	T_DRAG = T_DRAG + (2.0D0/RE)*DUDN*AREA2(I,J,K)*DSIN(THETA(I,J,K))
					
	T_LIFT_Y = T_LIFT_Y - (2.0D0/RE)*DUDN*AREA2(I,J,K)*DCOS(THETA(I,J,K))
	...
	IF(IPOST.EQ.1) THEN
		SH_S(I,J,K) = SH_S(I,J,K) + DABS(DUDN)*AREA2(I,J,K)/(AREA1(I,J,K)+AREA2(I,J,K))
...
!		CP_S(I,J,K)	= CP_S(I,J,K)
	...
	WRITE (*,210) RTIME,NU,CD,CL,CPB,CD_P,CD_V,CL_P,CL_V,CFL_MAX,DSQRT(CS_AVG),DSQRT(CT_AVG) ! PLOTTING
210	FORMAT(TR1,D16.9,TR1,D16.9,TR1,D16.9,TR1,D16.9,TR1,D16.9,TR1,D16.9,TR1,D16.9,TR1,D16.9,TR1,D16.9,TR1,D16.9,TR1,D16.9,TR1,D16.9)
...
!   SUBROUTINE FOR POST-PROCESSING
    IF(IPOST.EQ.1) THEN
		...
		U_RMS(IS:IE,JS:JE,KS:KE) = U_RMS(IS:IE,JS:JE,KS:KE) + U(IS:IE,JS:JE,KS:KE)**2
		V_RMS(IS:IE,JS:JE,KS:KE) = V_RMS(IS:IE,JS:JE,KS:KE) + V(IS:IE,JS:JE,KS:KE)**2
!		U_TA(:,:,:) = U_TA(:,:,:) + U(:,:,:)
!		V_TA(:,:,:) = V_TA(:,:,:) + V(:,:,:)
!		W_TA(:,:,:) = W_TA(:,:,:) + W(:,:,:)
!		P_TA(:,:,:) = P_TA(:,:,:) + P(:,:,:)
