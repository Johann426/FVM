...
!   CPB CALCULATION
	...
	I=NI
	    DUM_A = PG(I,1,2)        ! Far field pressure
	I=1
	    DUM_B = PG(I,NJ/2,2)           ! Base suction pressure
...
!------------------------------------------------------------------------------------------
	...
	I=NI
	    ...
	    DUM_A = DUM_A + DSQRT(U(I,J,K)**2+V(I,J,K)**2)*DABS(YG(I,J+1,K)-YG(I,J,K))
	    ELSE
	    DUM_B = DUM_B + DSQRT(U(I,J,K)**2+V(I,J,K)**2)*DABS(YG(I,J+1,K)-YG(I,J,K))
	    ENDIF
    ENDDO
	ENDDO
	
	WRITE (*,100) DUM_A,DUM_B,100.D0-(DUM_B-DUM_A)/DUM_A*100.D0
...
!   SUBROUTINE FOR POST-PROCESSING
    ...
    ! SUMMATION FOR TIME-AVERAGED RMS VALUE
        DUM_A = CD_TA/DBLE(NITER_POS)
        DUM_B = CL_TA/DBLE(NITER_POS)
        RMS_CD  = RMS_CD + (CD-DUM_A)**2
        RMS_CL  = RMS_CL + (CL-DUM_B)**2
    ! TIME-AVERAGED VALUE
        OPEN(UNIT=12,FILE='POST_NU_CD_CL.PLT',STATUS='OLD',ACCESS='APPEND')
        WRITE(12,*) RTIME,NU_TA/DBLE(NITER_POS),DUM_A,DUM_B,CL_AMP_TA/DBLE(NITER_POS)	&
			,CPB_TA/DBLE(NITER_POS),ST_TA/DBLE(NITER_POS),SQRT(RMS_CD/DBLE(NITER_POS)),SQRT(RMS_CL/DBLE(NITER_POS))
		CLOSE(12)
