...
!   CALCULATION OF DUDN on orthogonal grid
	...
	DO K=2,NKM1
	DO J=1,NJ-1
!    DUDN(I,J,K) = Q22(I,J,K)**0.5D0*(XIX(I,J,K)*U(I,J,K)+XIY(I,J,K)*V(I,J,K))  ! CELL-NODE BASED
	I=1 
!    DUDN(I,J,K) = &!<--- DU/DN = D(Usin0-Vcos0)/DN
!	            (U(I,J,K)*DSIN(THETA(I,J,K))-V(I,J,K)*DCOS(THETA(I,J,K))-VEL_ROT) & !DU
!	            /( (X(I,J,K)**2+Y(I,J,K)**2)**0.5D0 -0.5D0) !DN
