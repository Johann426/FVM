...
!                                                                           !
!     2D CYLINDER FORCED EXTERNAL FLOW  v1.9, Jin-Wook Lee                  !
!                                                                           !
...
!   ADVECTIVE EXPLICIT TERM : CONSERVATIVE FORM
    ...
    CALL NL(U_N,NLX)
    CALL NL(V_N,NLY)
    CALL NL(T,ADV)	!   THERMAL ADVECTIVE EXPLICIT TERM : CONSERVATIVE FORM

!   DIFFUSIVE EXPLICIT TERM : CONSERVATIVE FORM
    CALL DIF(U_N,DIFX)
    CALL DIF(V_N,DIFY)
    CALL DIF(T,DIFT)	!   THERMAL DIFFUSIVE EXPLICIT TERM : CONSERVATIVE FORM
