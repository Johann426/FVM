...
!   CALCULATION OF DUDN on orthogonal grid
	...
	P_DRAG = P_DRAG - 2.D0/DSQRT(Q11(I,J,K))*0.5D0*(P(I+1,J,K)-P(I-1,J,K))	&
			*XIX(I,J,K)/(XIX(I,J,K)**2+XIY(I,J,K)**2)**0.5*AREA(I,J,K)
	
	P_DRAG = P_DRAG - 2.D0/DSQRT(Q11(I,J,K))*0.5D0*(P(I+1,J,K)-P(I-1,J,K))	&
			*XIY(I,J,K)/(XIX(I,J,K)**2+XIY(I,J,K)**2)**0.5*AREA(I,J,K)
	
	T_DRAG = T_DRAG + (2.0D0/RE) *1.D0/DSQRT(Q22(I,J,K))*0.5D0*(U(I,J+1,K)-U(I,J-1,K))/DN1	&
			*ETX(I,J,K)/DSQRT(ETX(I,J,K)**2+ETY(I,J,K)**2)*AREA(I,J,K)

	T_DRAG = T_DRAG + (2.0D0/RE) *1.D0/DSQRT(Q22(I,J,K))*0.5D0*(V(I,J+1,K)-V(I,J-1,K))/DN1	&
			*ETX(I,J,K)/DSQRT(ETX(I,J,K)**2+ETY(I,J,K)**2)*AREA(I,J,K)

	T_LIFT = T_LIFT + (2.0D0/RE) *1.D0/DSQRT(Q22(I,J,K))*0.5D0*(U(I,J+1,K)-U(I,J-1,K))/DN1	&
			*ETY(I,J,K)/DSQRT(ETX(I,J,K)**2+ETY(I,J,K)**2)*AREA(I,J,K)

	T_LIFT = T_LIFT + (2.0D0/RE) *1.D0/DSQRT(Q22(I,J,K))*0.5D0*(V(I,J+1,K)-V(I,J-1,K))/DN1	&
			*ETY(I,J,K)/DSQRT(ETX(I,J,K)**2+ETY(I,J,K)**2)*AREA(I,J,K)
