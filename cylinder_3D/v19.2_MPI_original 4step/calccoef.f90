...
!   CALCULATION OF DUDN
	...
	T_DRAG = T_DRAG + (2.0D0/RE)*U(I,J,K)*DABS(DUM_B/DSQRT(DUM_A**2+DUM_B**2))/D_N*AREA1(I,J,K)		! (u,0,0) ! unit(A,B,C)
	
	T_LIFT_Y = T_LIFT_Y + (2.0D0/RE)*V(I,J,K)*DABS(DUM_A/DSQRT(DUM_A**2+DUM_B**2))/D_N*AREA1(I,J,K)	! (0,v,0) ! unit(A,B,C)
	...
	T_DRAG = T_DRAG + (2.0D0/RE)*U(I,J,K)*DABS(DUM_B)/DSQRT(DUM_A**2+DUM_B**2)/ D_N*AREA2(I,J,K)
	
	T_LIFT_Y = T_LIFT_Y + (2.0D0/RE)*V(I,J,K)*DABS(DUM_A)/DSQRT(DUM_A**2+DUM_B**2)/ D_N*AREA2(I,J,K)
...
105     FORMAT(TR1'ST =',F10.6,TR4'CL_AMP =',F10.6)
    ...
    CALL MPI_BCAST(ST,1,MPI_DOUBLE_PRECISION,0,NMCW,ERR)
