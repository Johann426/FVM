...
!	ENDDO
	...
	IF(RANK.EQ.0)		CALL MPI_ISEND(PLI(IS,J,KS1),IE-IS+1,MPI_DOUBLE_PRECISION,NPRC-1,1,MPI_COMM_WORLD,ISEND2,ERR)
	
	IF(RANK.EQ.NPRC-1)	CALL MPI_ISEND(PLI(IS,J,KE1),IE-IS+1,MPI_DOUBLE_PRECISION,0,1,MPI_COMM_WORLD,ISEND1,ERR)

	IF(RANK.EQ.NPRC-1)	CALL MPI_IRECV(PLI(IS,J,KE) ,IE-IS+1,MPI_DOUBLE_PRECISION,0,1,MPI_COMM_WORLD,IRECV2,ERR)
		
	IF(RANK.EQ.0)		CALL MPI_IRECV(PLI(IS,J,KS) ,IE-IS+1,MPI_DOUBLE_PRECISION,NPRC-1,1,MPI_COMM_WORLD,IRECV1,ERR)
