...
!-------------------------------------------
	...
	IF(RANK.EQ.0)		CALL MPI_ISEND(T(IS,JS,KS1),1,APLN,NPRC-1,1,MPI_COMM_WORLD,ISEND1,ERR)
	
	IF(RANK.EQ.NPRC-1)	CALL MPI_ISEND(T(IS,JS,KE1),1,APLN,0,1,MPI_COMM_WORLD,ISEND2,ERR)

	IF(RANK.EQ.NPRC-1)	CALL MPI_IRECV(T(IS,JS,KE) ,1,APLN,0,1,MPI_COMM_WORLD,IRECV1,ERR)
		
	IF(RANK.EQ.0)		CALL MPI_IRECV(T(IS,JS,KS) ,1,APLN,NPRC-1,1,MPI_COMM_WORLD,IRECV2,ERR)
