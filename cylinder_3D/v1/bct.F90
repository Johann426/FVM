	SUBROUTINE BCT

	USE COMDAT_SHARED
    
	IMPLICIT NONE
	SAVE
!-----------------------------------------
!   CYLINDER SURFACE
!-----------------------------------------
	DO K=0,NK
	DO J=0,NJ
	I=0
		T_F(SX1,J,K) = T_WALL

    AP(SX1,J,K) = AP(SX1,J,K) +AW(SX1,J,K)
    SU(SX1,J,K) = SU(SX1,J,K) +2.D0*AW(SX1,J,K)*T_F(SX1,J,K)
    AW(SX1,J,K) = 0.0D0
	! CELL-CENTER BOUNDARY VALUE
	T(I,J,K) = -T(I+1,J,K) + 2.0D0*T_F(SX1,J,K)
	ENDDO
	ENDDO
!-------------------------------------------
!   FAR FIELD INLET & OUTLET REGION
!-------------------------------------------
	DO K=0,NK
	DO J=0,NJ
	I=NI
	    IF (THETA(I,J,K).GE.0.5D0*PI .AND. THETA(I,J,K).LE.1.5D0*PI) THEN
	    T_F(NIM1,J,K) = TIN
	    ELSE
	    T_F(NIM1,J,K) = 0.5D0*(T_N(I,J,K)+T_N(I-1,J,K)) - DTIME*Uin       &
                    *(0.5D0*(T_N(I,J,K)+T_N(I-1,J,K))-T_N(I-1,J,K)) &
                    /(0.5D0*(  X(I,J,K)+  X(I-1,J,K)) - X(I-1,J,K))	
	    ENDIF
    AP(NIM1,J,K) = AP(NIM1,J,K) +AE(NIM1,J,K)
    SU(NIM1,J,K) = SU(NIM1,J,K) +2.D0*AE(NIM1,J,K)*T_F(NIM1,J,K)
    AE(NIM1,J,K) = 0.D0
    ! CELL-CENTER BOUNDARY VALUE
	T(I,J,K) = -T(I-1,J,K) + 2.0D0*T_F(NIM1,J,K)
    ENDDO
	ENDDO
!-------------------------------------------
!   BRANCH CUT REGION
!-------------------------------------------
	DO K=0,NK
	DO I=0,NI
    J=0
	    T(I,J,K) = T(I,NJM1,K)
	J=NJ
		T(I,J,K) = T(I,1,K)
	ENDDO
	ENDDO
!-------------------------------------------
!   PERIODIC WALL OF BOUNDARY
!-------------------------------------------
	DO J=0,NJ
	DO I=0,NI
	K=0
!		AP(I,J,SZ1) = AP(I,J,SZ1)-AB(I,J,SZ1)
!		AB(I,J,SZ1) = 0.D0
!		T(I,J,K) = T(I,J,K+1)	
		T(I,J,K) = T(I,J,NKM1)
	K=NK
!		AP(I,J,EZ1) = AP(I,J,EZ1)-AT(I,J,EZ1)
!		AT(I,J,EZ1) = 0.D0
!		T(I,J,K) = T(I,J,K-1)
		T(I,J,K) = T(I,J,1)
	ENDDO
	ENDDO
	
	END SUBROUTINE BCT