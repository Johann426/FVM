	...
	WRITE(CONTOUR(1:4),'(I4.4)') INT4(RTIME)
	
	WRITE(CONTOUR(6:7),'(I2.2)') INT4(RANK)
		
!	!   CELL CENTER RESULTS
!	OPEN(UNIT=100000+RANK,FILE=FILENAME,STATUS='UNKNOWN')
...
!   INTERPOLATION FOR NODAL RESULTS
	...
	DO I=IS1,IE
		...
		VOR_X(I,J,K) = 0.125D0*(VOR_X(I,J,K)+VOR_X(I,J-1,K)+VOR_X(I-1,J,K)+VOR_X(I-1,J-1,K)		&
		                +VOR_X(I,J,K-1)+VOR_X(I,J-1,K-1)+VOR_X(I-1,J,K-1)+VOR_X(I-1,J-1,K-1))
		VOR_Y(I,J,K) = 0.125D0*(VOR_Y(I,J,K)+VOR_Y(I,J-1,K)+VOR_Y(I-1,J,K)+VOR_Y(I-1,J-1,K)		&
		                +VOR_Y(I,J,K-1)+VOR_Y(I,J-1,K-1)+VOR_Y(I-1,J,K-1)+VOR_Y(I-1,J-1,K-1))
		VOR_Z(I,J,K) = 0.125D0*(VOR_Z(I,J,K)+VOR_Z(I,J-1,K)+VOR_Z(I-1,J,K)+VOR_Z(I-1,J-1,K)		&
		                +VOR_Z(I,J,K-1)+VOR_Z(I,J-1,K-1)+VOR_Z(I-1,J,K-1)+VOR_Z(I-1,J-1,K-1))
	ENDDO
	ENDDO
	ENDDO
...
!	BRANCH CUT
	...
	J=1
        ...
        VOR_X(I,J,K) = 0.5D0 * ( VOR_X(I,J,K) + VOR_X(I,NJ,K) )
        VOR_Y(I,J,K) = 0.5D0 * ( VOR_Y(I,J,K) + VOR_Y(I,NJ,K) )
        VOR_Z(I,J,K) = 0.5D0 * ( VOR_Z(I,J,K) + VOR_Z(I,NJ,K) )
		...
		WG(I,NJ,K) = WG(I,J,K)
		TG(I,NJ,K) = TG(I,J,K)
		VOR_X(I,NJ,K) = VOR_X(I,J,K)
		VOR_Y(I,NJ,K) = VOR_Y(I,J,K)
		VOR_Z(I,NJ,K) = VOR_Z(I,J,K)
	ENDDO
	ENDDO
