...
!-------------------------------------------
	...
	IF(RANK.EQ.0)		CALL MPI_ISEND(A(IS,JS,KS1),1,APLN,NPRC-1,1,COMM_NEW,ISEND1,ERR)
	
	IF(RANK.EQ.NPRC-1)	CALL MPI_ISEND(A(IS,JS,KE1),1,APLN,0,1,COMM_NEW,ISEND2,ERR)

	IF(RANK.EQ.NPRC-1)	CALL MPI_IRECV(A(IS,JS,KE) ,1,APLN,0,1,COMM_NEW,IRECV1,ERR)
		
	IF(RANK.EQ.0)		CALL MPI_IRECV(A(IS,JS,KS) ,1,APLN,NPRC-1,1,COMM_NEW,IRECV2,ERR)
