...
!   CPB CALCULATION
	...
	DO J=JS1,JE1
	I=IS1
!	L1 = DSQRT((X(I,J,K)   -XG(I,J  ,K))**2+(Y(I,J,K)   -YG(I,J  ,K))**2.D0)
!	L2 = DSQRT((X(I,J,K)   -XG(I,J+1,K))**2+(Y(I,J,K)   -YG(I,J+1,K))**2.D0)
!	L3 = DSQRT((XG(I,J+1,K)-XG(I,J  ,K))**2+(YG(I,J+1,K)-YG(I,J  ,K))**2.D0)
	...
	L1 = DSQRT((X(I,J,K)-DUM_A)**2+(Y(I,J,K)-DUM_B)**2)
	L2 = DSQRT((X(I,J,K)-DUM_C)**2+(Y(I,J,K)-DUM_D)**2)
	L3 = DSQRT((DUM_C-DUM_A)**2+(DUM_D-DUM_B)**2)
...
!	LB_ = Y(I,J,K)-LA_*X(I,J,K)
    ...
    DN2=DSQRT((Y(I,J,K)-Y1)**2+(X(I,J,K)-X1)**2)
